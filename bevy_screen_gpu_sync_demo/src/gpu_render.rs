...
// Copies GPU data to stable buffer first, then draws from buffer to eliminate flicker
...
/// Main plugin for GPU rendering with pre-buffer strategy
...
/// Draw an ANIMATED red square test pattern directly to the render target
/// The square moves diagonally to test sync under motion
