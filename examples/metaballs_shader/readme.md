...
Edit the [`frag_shader.wgsl`](/examples/metaballs_shader/src/frag_shader.wgsl) to create custom shaders that run to the screen.

Compile on the Mac and send to Pi:

```bash
cd examples/metaballs_shader
./deploy.sh
```
